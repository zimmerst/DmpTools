...
printf "\n>>>> Execute unige_pbs_check_jobs.bash \n\n"
...
n_submitted=`qstat -u ${user} | grep ${queue} | grep job_${skim_version} | grep -v '\.e' | grep -v '\.o' | grep -v ' C ' | grep -v ' E ' | wc -l`
n_running=`qstat -u ${user} | grep ${queue} | grep job_${skim_version} | grep -v '\.e' | grep -v '\.o' | grep ' R ' | wc -l`
